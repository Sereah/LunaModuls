...
Java_com_lunacattus_llm_jni_LlmNative_nativeSetQnnEnv(JNIEnv *env, jclass,
    ...
    LOGI("%s: nativeLibDir=%s", __func__, path);

    // ADSP_LIBRARY_PATH：DSP/NPU 库搜索路径（Qualcomm FastRPC 使用）
    int ret1 = setenv("ADSP_LIBRARY_PATH", path, 1);

    // LD_LIBRARY_PATH：标准 Linux 动态库搜索路径
    int ret2 = setenv("LD_LIBRARY_PATH", path, 1);
    ...
    LOGI("%s: QNN 环境已设置 (ADSP_LIBRARY_PATH=%s)", __func__, path);
    return 0;
}
